    ...
    void variant::visit(const visitor &v) const {
        switch (get_type()) {
            ...
            case blob_type:
                ...
                return;
            default:
                std::cout << "assert_exception" << "Invalid Type / Corrupted Memory" << std::endl;
        }
    }
    ...
    int64_t variant::as_int64() const {
        switch (get_type()) {
            ...
            case null_type:
                return 0;
            default:
                std::cout << "bad_cast_exception,Invalid cast from ${type} to int64 ,type:" << get_type() << std::endl;
        }
        return -1;
    }
        ...
        try {
            switch (get_type()) {
                ...
                case null_type:
                    return 0;
                default:
                    std::cout << "bad_cast_exception, Invalid cast from ${type} to uint64, type:" << get_type()
                              << std::endl;
    ...
    double variant::as_double() const {
        switch (get_type()) {
            ...
            case null_type:
                return 0;
            default:
                std::cout << "bad_cast_exception, Invalid cast from  to double ,type:" << get_type() << std::endl;
        }
        return 0.0;
    }
        ...
        switch (get_type()) {
            case string_type: {
                ...
                if (s == "false")
                    return false;
                std::cout
                        << "bad_cast_exception,Cannot convert string to bool (only \"true\" or \"false\" can be converted"
                        << std::endl;
            }
            case double_type:
            ...
            case null_type:
                return false;
            default:
                std::cout << "bad_cast_exception, Invalid cast from ${type} to bool type:" << get_type() << std::endl;
        }
        return false;
    }
    ...
    string variant::as_string() const {
        switch (get_type()) {
            ...
            case null_type:
                return string();
            default:
                std::cout <<"bad_cast_exception, Invalid cast from ${type} to string type" << get_type() <<std::endl;
        }
        static variant v;
        return   **reinterpret_cast<const const_string_ptr *>(&v);;
    }
...
/// @throw if get_type() != array_type | null_type
    variants &variant::get_array() {
        ...
        std::cout <<"bad_cast_exception, Invalid cast from ${type} to string Array" << get_type() <<std::endl;

        static variant v;
        return **reinterpret_cast<variants **>(&v);
    }
    ...
    blob &variant::get_blob() {
        ...
        std::cout <<"bad_cast_exception, Invalid cast from ${type} to string Blob" << get_type() <<std::endl;

        static variant v;
        return   **reinterpret_cast< blob **>(&v);;
    }
    ...
    const blob &variant::get_blob() const {
        ...
        static variant v;
        return **reinterpret_cast<const const_blob_ptr *>(&v);
    }
    ...
    blob variant::as_blob() const {
        switch (get_type()) {
            ...
            case object_type:
            case array_type:
                std::cout << "bad_cast_exception, Invalid cast from ${type} to Blob, type" << get_type() << std::endl;
            default:
...
/// @throw if get_type() != array_type
    const variants &variant::get_array() const {
        ...
        std::cout << "bad_cast_exception, Invalid cast from ${type} to Array type:" << get_type() << std::endl;

        static variant v;
        return **reinterpret_cast<const const_variants_ptr *>(&v);
    }
    ...
    const string &variant::get_string() const {
        if (get_type() == string_type)
            return **reinterpret_cast<const const_string_ptr *>(this);
        std::cout << "bad_cast_exception, Invalid cast from type '${type}' to string,type :" << get_type() << std::endl;
...
// TODO: warn on overflow?
    ...
    void to_variant(const std::vector<char> &var, variant &vo) {
        assert(var.size() <= MAX_SIZE_OF_BYTE_ARRAYS);
        if (var.size())
    ...
    void from_variant(const variant &var, std::vector<char> &vo) {
        ...
        assert(str.size() <= 2 * MAX_SIZE_OF_BYTE_ARRAYS); // Doubled because hex strings needs two characters per byte
        assert(str.size() % 2 == 0);
